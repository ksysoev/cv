...
3. Build Hugo site
4. Render a print-first HTML (`public/cv-print.html`) from `resume.yaml` data
5. Generate a downloadable PDF artifact at `public/cv.pdf` with Puppeteer
6. Deploy `public/` using GitHub Pages Actions

PDF contract:
- Published filename/location: `public/cv.pdf` (served at `…/cv.pdf` — `/cv.pdf` on user/org pages, `/<repo>/cv.pdf` on project pages)
- Intermediate print template: `public/cv-print.html` (generated by `scripts/render-cv-print.mjs`)
